# Functions in this file: igc_alpha() igc_delta() igc_triple() igc_clumped()
...
#'
#' @description
#' `igc_alpha()` calculates the isotope fractionation factors
#' between two phases.
#'
#' @param temp Carbonate growth temperature (°C).
#' @param isotope Isotope system.
#' @param phases Phases.
#' @param eq Equation used for the calculations. See details.
#' @param log Calculate elena?
#'
#' @details
...
dat = read.xlsx("igc_equations.xlsx")
igc_alpha = function(temperature, isotope, phases, eq, log) {

  TinK = temperature + 273.15

  # Get the equation parameters (can result in an empty data frame)
  params = filter(dat, isotope == iso & eq == equ)

  # Check if input parameters exist in the database
  if (isFALSE(any(stringr::str_detect(dat$isotope, iso))) ||
      dim(dat)[1] == 0)  {
    stop("Unknown isotope system. For help type ?igc_equations")
  } else if (isFALSE(any(stringr::str_detect(params$eq, str_c("\\b", equ, "\\b"))))) {
    stop("Unknown equation. For help type ?igc_equations")
  } else {

  }

  # Calculate alpha based on the input parameters
  if (params$form == "1/T")  {
    alpha = exp(params$m * 1 / TinK  - params$b)
  } else if (params$form == "1000/T2") {
    alpha = exp(params$m * 1000 / TinK ^ 2 - params$b)
  } else {
    stop("ERROR")
  }

  # Calculate 1000ln(alpha) value if log=T
  if (log) {
    1000 * log(alpha)
  } else {
    alpha
